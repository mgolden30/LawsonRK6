function [U, expm_times, rk6_times] = SimpleLawson_RK6(a,b,u,t,steps,m,k,mask)
  %{
  Accomplish the same output as SLRK4, but for a RK6 scheme with equally
  spaced anmd ordered c_i
  ...
  for i = 1:steps
    ...
    H_square = H(1:m,1:m);
    expH = expm(H_square * h/6);
    expm_times(i) = toc;

    %project and exponentiate
    e = @(v,j) Q(:,1:m) * expH * Q(:,1:m)' * v;
